...
- `/upload` error row — **Resolved (2026-08-17).** The client cap now matches the backend `max_file_size` (512 MiB) through the single source of truth `apps/web/src/lib/upload-limits.ts` (`MAX_UPLOAD_BYTES` / `MAX_UPLOAD_LABEL`), and both the limit label and the humanized size render via `humanizeBytes`, so they share one unit — no MiB-vs-MB drift. Previously a 113,798,180-byte file was rejected as "exceeds 100MB limit (108.5 MB)" while the cap was actually 100 MiB, and the stuck 100 MB client cap also blocked valid 3D volumes under the documented 512 MB limit
