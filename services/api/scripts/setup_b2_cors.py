...
existing CORS rules are preserved and one rule (ID `nnunet-direct-upload`) is
...
def _client():
    ...
    # already knows from B2_REGION (the S3 endpoint is derived from it).
    return boto3.client(
        "s3",
        ...
        config=Config(signature_version="s3v4", user_agent_extra="b2ai-nnunet-3d-medical-image-segmentation"),
    )
